            ...
            <div class="col-lg-6">
                                ...
                                @foreach($product->bundleItems as $item)
                                    <div class="list-group-item d-flex align-items-center p-2 border-bottom">
                                        <div class="flex-shrink-0 me-3">
                                            <img src="{{ $item->image ? (Str::startsWith($item->image, 'http') ? $item->image : asset('storage/' . $item->image)) : 'https://placehold.co/50x50' }}" 
                                                 alt="{{ $item->name }}" 
                                                 class="rounded border"
                                                 style="width: 50px; height: 50px; object-fit: cover;">
                                        </div>
                                        <div class="flex-grow-1">
                                            <h6 class="mb-0 text-truncate" style="max-width: 250px;" title="{{ $item->name }}">{{ $item->name }}</h6>
                                            <small class="text-muted">{{ $item->pivot->quantity }} unidade(s)</small>
                                        </div>
                                        <div class="flex-shrink-0 ms-2">
                                            <a href="{{ route('shop.show', $item->slug) }}" class="btn btn-sm btn-light text-primary" target="_blank" title="Ver Produto">
                ...
                @if($product->stock > 0)
                    <div class="mb-4">
                        ...
                        @include('livewire.shop.add-to-cart-button', ['product' => $product])
        ...
        <div class="mt-5 pt-4 border-top">
            ...
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4">
                @foreach($relatedProducts as $related)
                    <div class="col">
                        @include('livewire.shop.product-card', ['product' => $related])
                    </div>
                @endforeach
            </div>
        </div>
