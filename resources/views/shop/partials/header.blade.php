        ...
        <div class="container" style="max-width: 1200px;">
            ...
            <div class="d-flex order-lg-3 me-2">
                ...
                <div class="nav-item me-2 position-relative" 
                     x-data="{
                        latestStoryTimestamp: '{{ $storyStatus->latestStoryTimestamp }}',
                        hasActiveStories: {{ $storyStatus->hasActiveStories ? 'true' : 'false' }},
                        isUnseen: false,
                    ...
                    <button class="btn btn-outline-warning btn-icon-shape rounded-circle position-relative p-0 d-flex align-items-center justify-content-center" 
                        ...
                        <div class="d-flex align-items-center justify-content-center w-100 h-100 rounded-circle overflow-hidden">
                            @auth
                                @if(Auth::user()->avatar)
                                    <img src="{{ Auth::user()->avatar }}" class="w-100 h-100 object-fit-cover" alt="Avatar">
                                @else
                                    <span class="fw-bold fs-6">{{ substr(Auth::user()->name, 0, 1) }}</span>
                                @endif
                            @endauth
            ...
            <div class="collapse navbar-collapse order-lg-2">
                ...
                <ul class="navbar-nav mb-2 mb-lg-0">
                    <li class="nav-item"><a class="nav-link text-white active" href="{{ route('shop.index') }}" style="font-weight: 500;">Destaques</a></li>
    ...
    <div class="offcanvas offcanvas-end bg-white" tabindex="-1" id="offcanvasNav" 
        ...
        <div class="offcanvas-body">
            <ul class="navbar-nav">
                <!-- Links Principais -->
                <li class="nav-item">
                    <a class="nav-link active fs-5" href="{{ route('shop.index') }}">
                        <i class="bi bi-house-fill me-2"></i>Início
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link fs-5" href="{{ route('shop.index') }}?filter=new">
                        <i class="bi bi-stars me-2"></i>Lançamentos
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link fs-5 text-danger" href="{{ route('shop.index') }}?filter=offers">
                ...
                @if($categories->count() > 0)
                    ...
                    @foreach($categories as $category)
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('shop.category', $category->slug) }}">
                                {{ $category->name }}
                            </a>
                        </li>
                    @endforeach
                ...
                <!-- Ações Rápidas -->
                <li class="nav-item border-top mt-3 pt-3">
                    <a class="nav-link text-muted" href="{{ route('customer.account.profile.edit') }}">
                        <i class="bi bi-person-circle me-2"></i>Minha Conta
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link text-muted" href="{{ route('shop.index') }}">
    ...
    <div class="shadow-sm d-none d-md-block" style="background-color: {{ $storeSettings['color_category_bar'] ?? '#f0f8ff' }} !important; border-top: 1px solid rgba(0,0,0,0.05) !important;">
        <div class="container py-2" style="max-width: 1200px;">
            <ul class="nav justify-content-center">
                ...
                <li class="nav-item"><a class="nav-link text-dark fw-bold" href="{{ route('shop.category', 'praia') }}" style="transition: color 0.3s;" onmouseover="this.style.color='#0d6efd'" onmouseout="this.style.color='#212529'">ModaPraia</a></li>
                <li class="nav-item"><a class="nav-link text-dark fw-bold" href="{{ route('shop.category', 'croche') }}" style="transition: color 0.3s;" onmouseover="this.style.color='#0d6efd'" onmouseout="this.style.color='#212529'">ModaCrochê</a></li>
                <li class="nav-item"><a class="nav-link text-dark fw-bold" href="{{ route('shop.category', 'suplementos') }}" style="transition: color 0.3s;" onmouseover="this.style.color='#0d6efd'" onmouseout="this.style.color='#212529'">LosfitNutri</a></li>
                <li class="nav-item"><a class="nav-link text-danger fw-bold" href="{{ route('shop.index', ['filter' => 'offers']) }}" style="transition: color 0.3s;" onmouseover="this.style.color='#dc3545'" onmouseout="this.style.color='#dc3545'">Promoções</a></li>
            </ul>
        </div>
    </div>
</header>
