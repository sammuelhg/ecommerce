<div>
    <div class="row g-4 mb-5">
        ...
        <div class="col-md-3">
            <div class="card shadow-sm border-start border-4 border-primary h-100">
                <div class="card-body">
                    <h6 class="text-uppercase text-muted small fw-bold">Total de Leads</h6>
                    <h2 class="mb-0 text-primary fw-bold">{{ number_format($totalLeads) }}</h2>
                    <small class="text-success fw-bold">
                        <i class="bi bi-arrow-up"></i> {{ $newLeadsToday }} hoje
                    </small>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-3">
            <div class="card shadow-sm border-start border-4 border-success h-100">
                <div class="card-body">
                    ...
                    <small class="text-muted">Global (Estimada)</small>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-3">
            <div class="card shadow-sm border-start border-4 border-info h-100">
                <div class="card-body">
                    ...
                    <h2 class="mb-0 text-info fw-bold">{{ $recentCampaigns->where('status', 'sent')->count() }}</h2>
                </div>
            </div>
        </div>
    </div>
            ...
            <a href="{{ route('admin.marketing.contacts') }}" class="text-decoration-none">
                <div class="card text-center h-100 shadow-sm hover-card border-primary-subtle">
                    <div class="card-body d-flex flex-column justify-content-center py-5">
                        ...
                        <h4 class="card-title fw-bold mb-0 text-dark">Contatos</h4>
                        <p class="text-muted small mt-2 mb-0">Gerenciar Leads e Clientes</p>
                    </div>
                </div>
            </a>
        </div>

        <!-- Campanhas (Legacy/New) -->
        <div class="col-md-4">
            <a href="{{ route('admin.campaigns.index') }}" class="text-decoration-none">
                <div class="card text-center h-100 shadow-sm hover-card border-success-subtle">
                    <div class="card-body d-flex flex-column justify-content-center py-5">
                        ...
                        <h4 class="card-title fw-bold mb-0 text-dark">Campanhas</h4>
            ...
            <a href="{{ route('admin.emails.preview.dashboard') }}" class="text-decoration-none">
                <div class="card text-center h-100 shadow-sm hover-card border-warning-subtle">
                    <div class="card-body d-flex flex-column justify-content-center py-5">
                        ...
                        <p class="text-muted small mt-2 mb-0">Visualizar modelos</p>
                    </div>
                </div>
            </a>
        </div>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-header py-3 border-bottom">
            <h5 class="mb-0 card-title fw-bold"><i class="bi bi-bar-chart-line me-2"></i>Aberturas Recentes</h5>
        </div>
        <div class="card-body">
            @if(empty($chartData))
    ...
    <div class="card shadow-sm border-0 mt-5">
        <div class="card-header bg-white py-3 border-bottom">
            <h5 class="mb-0 card-title fw-bold"><i class="bi bi-list-check me-2"></i>Últimas Campanhas</h5>
        </div>
            ...
            <table class="table table-hover align-middle mb-0">
                <thead class="bg-light text-secondary small text-uppercase">
                    <tr>
                        ...
                        <th class="text-center" style="width: 15%;">Destinatários</th>
                ...
                <tbody class="border-top-0">
                    @forelse($recentCampaigns as $campaign)
                        <tr>
                            <td class="ps-4">
                                <a href="{{ route('admin.marketing.report', $campaign->id) }}" class="fw-bold text-dark d-block text-decoration-none hover-underline">
                                    {{ $campaign->subject }}
                                </a>
                                <small class="text-muted">{{ $campaign->created_at->format('d/m/Y H:i') }}</small>
                                <div class="mt-1">
                                    @if(isset($campaign->status) && method_exists($campaign->status, 'label'))
                                        <span class="badge bg-secondary-subtle text-secondary border border-secondary">{{ $campaign->status->label() }}</span>
                                    @else
                                         <span class="badge bg-secondary-subtle text-secondary">{{ $campaign->status ?? 'N/A' }}</span>
                                    @endif
                                </div>
                            </td>
                                            ...
                                            <div class="d-flex align-items-center justify-content-between p-2 rounded bg-light border">
                                                <div class="d-flex align-items-center overflow-hidden">
                                                    ...
                                                    <div class="text-truncate" style="max-width: 200px;">
                                                        ...
                                                        <div class="text-muted" style="font-size: 0.7rem;">+{{ $email->delay_in_hours }}h</div>
                                                    </div>
                                                </div>
                                                <div class="text-end">
                                                    <div class="d-flex align-items-center gap-2">
                                                        <i class="bi bi-envelope-open text-muted title="Aberturas"></i>
